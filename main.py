...
class st_variables_container:
    ...
    def __init__(self):
        with st.container():
            ...
            st.selectbox( 'Choose the appropriate YOLOV7 Model',
                          list( config.STYLES.keys() ), key='model_style' )
            st.slider(
                "Confidence threshold", 0.0, 1.0, 0.5, 0.05, key='confidence_threshold',
                 help="This confidence score shows the probability of the image being detected \
                        correctly by the algorithm and is given as a percentage. The scores are \
                        taken on the mean average precision at different IoU (Intersection over Union) thresholds"
            )
            # st.caption( '[SORT](https://github.com/abewley/sort) Tracking Algorithm' )
            st.caption( 'SORT Tracking Algorithm')
            st.slider(
                "Tracking Age (frames)", 0, 20, 10, 1, key='track_age',help='Maximum number of frames to keep \
                                            alive a track without associated detections.'
            )
            st.slider(
                "Tracking hits", 0, st.session_state.track_age, 3, 1, key='tracking_hits',help="Minimum number of associated\
                                        detections before track is initialised."
            )
            st.slider(
                "IOU threshold", 0.0, 1.0, 0.7, 0.1, key='iou_thres',help="Minimum IOU for match."
            )
    ...
    if file is not None:
        ...
        if width > 1920 or height > 1080:
            ...
            if st.button( 'Detect' ):
                ...
                # print(f"Configuration path:{config.HERE}")
                ...
                process = subprocess.Popen( [ffmpeg_source] + args, stdin=subprocess.PIPE, shell = True)
